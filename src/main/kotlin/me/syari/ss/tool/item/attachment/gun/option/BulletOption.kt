...
data class BulletOption(
    ...
    fun shoot(player: Player, isSneak: Boolean, isScope: Boolean, hitEvent: (LivingEntity, Entity, Boolean) -> Unit) {
        ...
        runRepeatTimes(toolPlugin, burstDelay.toLong(), burstAmount) {
            ...
            bullet.setMetadata(sourcePlayerMetaDataKey, FixedMetadataValue(toolPlugin, player))
            bullet.velocity = direction.multiply(bulletSpeed)
            val taskList = mutableSetOf<CustomTask>()
            runLater(toolPlugin, remove.toLong()) {
                bullet.remove()
                taskList.forEach { it.cancel() }
            }?.let { taskList.add(it) }
            if (particleList != null) {
                runTimer(toolPlugin, particleList.getRequireTime()) {
                    particleList.spawn(bullet)
                }?.let { taskList.add(it) }
            }
            runTimer(toolPlugin, 1) {
    ...
    companion object {
        ...
        private const val sourcePlayerMetaDataKey = "ss-tool-gun-source-player"
    }
}